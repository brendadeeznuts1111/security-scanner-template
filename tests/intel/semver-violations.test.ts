...
test('checkPolicySemverViolations enforces packages and blocked ranges', () => {
	...
	const violations = checkPolicySemverViolations({'lodash': '4.17.20', 'bad-pkg': '1.5.0'}, policy);
	expect(violations.some(v => v.source === 'policy-allowed' && v.package === 'lodash')).toBe(true);
	expect(violations.some(v => v.source === 'policy-blocked' && v.package === 'bad-pkg')).toBe(true);
});

test('checkThreatFeedViolations matches feed version ranges', () => {
	const violations = checkThreatFeedViolations({axios: '0.21.1'}, [
		{
			id: 'CVE-2026-9999',
			package: 'axios',
			versionRange: '<1.0.0',
